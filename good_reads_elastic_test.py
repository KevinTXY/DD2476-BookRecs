...
def indexer(file = ""):
    ...
    if file == "":
        ...
        for line in lines:
            l+=1
            print("reading line:" + str(l) +" of " + str(len(lines)))
            es.indices.refresh(index=index)
            es.indices.refresh(index=index2)
            line = line.strip("\n")
            ...
            if line != "":
                i = int(line)
                #checks if id already in index
                if es.search(index=index,body= {"query": {"match":{"_id":i}}})["hits"]["hits"] == []:
                    try:
                        book = gc.book(i)
                        print("indexing: " + book.title)
                        form = book._book_dict["format"]
                        ...
                        print(e)
                        print("book id " + str(line) +" no good")
                else:
                    print("book " + str(i) + " already in index")
