...
class searcher:
    ...
    def choose(self,test):
        ...
        while search_term != "done()":
            res = self.es.search(index="good_reads_test", body={"query": {"match": {"title" :search_term } },"size":"15"})
            #if any hits
            if res['hits']["total"]['value'] > 0:
                
                hits = [h['_source']['title'] for h in res['hits']['hits']]
                #languages =  [self.gc.book(int(h["_id"])).language_code for h in res["hits"]["hits"]]
                #languages = ["NA" if l == None else l for l in languages]
                ids = [h["_id"] for h in res["hits"]["hits"]]
                for i in range(len(hits)):
                    print(str(i) + " " + str(hits[i]) + " " +ids[i])
    ...
    def search(self, books):
        ...
        for book in books:
            ...
            if(book["_source"]["description"] != None):
                ...
                res = self.es.search(index="good_reads_test", body={"query": {"match": {"description" :book["_source"]["description"]} },"size":"100"})
